module ThermalElementList1DMOD
  use tools
  use DebuggerMOD

  use Element1DMOD
  use Element1DPtrMOD
  use ThermalLinLineElementMOD
  use ThermalQuadLineElementMOD
  ...
  implicit none
  private
  public :: ThermalElementList1DTYPE, thermalElementList1D
  type :: ThermalElementList1DTYPE
     private
  ...
  end type ThermalElementList1DTYPE
  ...
  type(ThermalLinLineElementTYPE) , save, dimension(:), allocatable, target :: linLineElem
  type(ThermalQuadLineElementTYPE), save, dimension(:), allocatable, target :: quadLineElem
  
contains

  type(ThermalElementList1DTYPE) function constructor(isQuadratic, nElem, nGauss)
    implicit none
  ...
  subroutine init(this, isQuadratic, nElem, nGauss)
    implicit none
    class(ThermalElementList1DTYPE), intent(inout) :: this
    integer(ikind), intent(in) :: isQuadratic
    integer(ikind), intent(in) :: nElem
    integer(ikind), intent(in) :: nGauss
...
end module ThermalElementList1DMOD
