  ...
  subroutine calculateFlux(this, problem)
    ...
    do iElem = 1, nElem
       ...
       do iGauss = 1, integrator%ptr%integTerms
          ...
          allocate(dsf(1,element1D%getnPoint()))
          dsf(1,:) = integrator%ptr%dShapeFunc(iGauss,1,:)
          q = 0.d0
          do i = 1, nPoint
             q = q + dsf(1,i)*problem%dof(element1D%getPointID(i))/jacobian1D
          end do
    ...
    do iElem = 1, nElem
       ...
       do iGauss = 1, integrator%ptr%integTerms
          ...
          allocate(dsf(2,element2D%getnPoint()))
